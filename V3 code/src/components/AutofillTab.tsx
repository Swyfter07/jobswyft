...
export function AutofillTab({ resumeData, activeResume, jobData, refreshKey = 0 }: AutofillTabProps) {
    ...
    const detectFields = async () => {
        ...
        try {
            ...
            const response = await chrome.tabs.sendMessage(tab.id, { action: 'DETECT_FORM_FIELDS' });
            ...
            if (response?.success && response.fields) {
                ...
                if (!resumeFieldOverrideId) {
                    const autoDetected = response.fields.find((f: DetectedField) => f.category === 'resume');
                    if (autoDetected) {
    ...
    const handleFill = async () => {
        ...
        try {
            ...
            const fieldValues = detectedFields
                ...
                .filter(Boolean) as Array<{ selector: string; value: string }>;

            // RESUME UPLOAD: Hybrid V1 Strategy (Direct Injection into MAIN world)
                ...
                await chrome.scripting.executeScript({
                    ...
                    func: (suggestedSelector: string, fileData: string, fileName: string) => {
                        ...
                        const isAutofill = (el: HTMLElement) => {
                            const text = (el.id + ' ' + el.name + ' ' + el.className + ' ' + (el.parentElement?.innerText || '')).toLowerCase();
                        ...
                        if (suggested && !isAutofill(suggested)) {
                            ...
                            element = allInputs.find(input => {
                                const ctx = (input.id + ' ' + input.name + ' ' + (input.parentElement?.innerText || '') + ' ' + (document.querySelector(`label[for="${input.id}"]`)?.innerText || '')).toLowerCase();
